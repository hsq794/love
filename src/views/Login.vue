    ...
    <div
      ...
      <el-form :model="user" :rules="rules" ref="userForm">
        <el-form-item prop="cname">
          <el-input size="medium" style="margin: 10px 0" prefix-icon="el-icon-user" v-model="user.cname"></el-input>
        </el-form-item>
        <el-form-item prop="cpassword">
          <el-input size="medium" style="margin: 10px 0" prefix-icon="el-icon-lock" show-password
                    v-model="user.cpassword"></el-input>
        </el-form-item>
  ...
  methods: {
    login() {
      this.$refs['userForm'].validate((valid) => {
        if (valid) {  // 表单校验合法
          this.request.post("/controller/login", this.user).then(res => {
            if (res.code === '200') {
